		...
		public async Task<IHttpActionResult> ChangeForgotPassword(SetForgotPasswordBindingModel model)
		{
			try
			{
				...
				using (RiscoContext ctx = new RiscoContext())
				{
					...
					var user = ctx.Users.Include(x=>x.ResetPasswordCode).FirstOrDefault(x => x.Email == model.Email  && x.IsDeleted == false);
                    if (user != null)
                    {
						var code = user.ResetPasswordCode.FirstOrDefault(x => x.Code == model.Code);
                        if (code != null)
                        {
                            if (code.ExpiryDate.Date < DateTime.UtcNow.Date && !code.IsExpired)
                            {
                                return Ok(new CustomResponse<Error> { Message = "Forbidden", StatusCode = (int)HttpStatusCode.Forbidden, Result = new Error { ErrorMessage = "User code has been expired." } });
                            }
                            else
                            {
							...
							return Ok(new CustomResponse<Error> { Message = "Forbidden", StatusCode = (int)HttpStatusCode.Forbidden, Result = new Error { ErrorMessage = "Invalid code" } });


					}
					else
                        return Ok(new CustomResponse<Error> { Message = "Forbidden", StatusCode = (int)HttpStatusCode.Forbidden, Result = new Error { ErrorMessage = "User with email doesnt exist." } });
			...
			try
			{
				...
				else if (string.IsNullOrEmpty(model.Email))
				{
					return Ok(new CustomResponse<Error>
					{
						...
						Result = new Error { ErrorMessage = "Invalid Email." }
					});
				}
		...
		public async Task<IHttpActionResult> EditUserAddress(EditUserAddressBindingModel model)
		{
			try
			{
				...
				using (RiscoContext ctx = new RiscoContext())
				{
					...
					{
							return Ok(new CustomResponse<Error> { Message = Global.ResponseMessages.NotFound, StatusCode = (int)HttpStatusCode.NotFound, Result = new Error { ErrorMessage = Global.ResponseMessages.GenerateInvalid("User") } });
					}
			...
			try
			{
				using (RiscoContext ctx = new RiscoContext())
				{
					...
					if (user != null)
					{
						...
						ctx.ResetPasswordCode.Add(new ResetPasswordCode
						{
						...
						const string body = "Reset Password Code : ";
						...
						return Ok(new CustomResponse<string> { Message = Global.ResponseMessages.Success, StatusCode = (int)HttpStatusCode.OK, Result = "Please check your email and follow instructions regarding resetting password." });
					}
					else
					{
